...
| `cloud-upstreams.ts` (#6548, 2026-05-22) | 로컬 인스턴스 → cloud upstream 회사 동기화 — 연결·push run·활성화 | `GET /api/cloud-upstreams`, `POST /api/cloud-upstreams/connect/{start,finish}`, `POST /api/cloud-upstreams/:connectionId/push-runs(/preview)`, `GET/POST /api/cloud-upstreams/:connectionId/push-runs/:runId(/cancel/activation)` |
...
라우트가 *얇은 컨트롤러* 라면, services/ 는 *두꺼운 도메인* 이다. 디렉터리 구성을 5개 도메인으로 묶으면 다음과 같다. **코드 1** 이 그 5개 도메인 트리이며, 각 군집의 KB 표기가 *어디에 도메인 무게가 쏠려 있는지* 를 그대로 보여 준다 — `routes/issues.ts` 205 KB(§2 표 1)·`company-portability.ts` 178 KB 가 가장 큰 두 봉우리.
...
**신규 도메인 — Cloud Upstream sync(#6548, 2026-05-22)** — `services/cloud-upstreams.ts` 가 +1298 LOC 로 모놀리식 추가되며 한 도메인 군집이 통째로 들어왔다. 이는 로컬 Paperclip 인스턴스를 **cloud upstream**(paperclip.ing 또는 셀프 호스트 cloud) 과 연결해 회사·이슈·run 산출물을 push 동기화하는 통로다. 데이터 모델 차원에서 새 테이블 `cloud_upstreams` 가 추가됐고(마이그레이션 `0089_cloud_upstreams`, 스키마 `packages/db/src/schema/cloud_upstreams.ts:1-75`), shared 타입 `packages/shared/src/types/cloud-upstream.ts` 가 연결·push run 페이로드를 zod 로 표현한다. UI 면은 보드의 `/company/settings/cloud-upstream` 페이지(`ui/src/pages/CloudUpstream.tsx`, 646 LOC) 와 ux-lab 변형(`CloudUpstreamUxLab.tsx`, 822 LOC) 으로 노출된다. CLI 측면(`cli/src/commands/client/cloud.ts`, 721 LOC + cloud-store/cloud-transfer) 으로도 같은 도메인이 들어와 — *Paperclip 이 멀티 인스턴스 회사 동기화 패턴을 처음 코어에 받아들였다는 점에서* 거버넌스·portability 표면과 함께 봐야 할 신규 축이다.
