...
`packages/db/src/schema/` 에는 **86개의 `pgTable()` 정의가 78개 스키마 파일** 에 분산되어 있다(`packages/db/src/schema/index.ts:1-78`). 모두를 한 그림에 담는 것은 무의미하므로, 보드 운영 흐름을 따라가는 데 필수적인 12\~13개 테이블만 추려 그림 4로 묶었다.
...
`AGENTS.md` Core Engineering Rule #1이 강제하는 단일 불변항은 명료하다 — *"Every domain entity should be scoped to a company and company boundaries must be enforced in routes/services."* 80여 개 테이블 중 60여 개가 `company_id uuid not null references companies.id` 컬럼을 들고 있다. 라우트 레이어의 미들웨어가 모든 mutating call에 대해 회사 멤버십·권한을 검사한다.
...
표 1 을 한 묶음으로 보면 *7개 테이블이 곧 control plane 의 7개 어휘* 라는 점이 드러난다 — 회사(`companies`)가 모든 데이터의 루트, 그 안에 *행위자* (`agents`)와 *작업 단위* (`issues`)가 살고, 모든 *행위* 는 `cost_events` 와 `activity_log` 가 두 채널로 기록하며, *판단* 은 `approvals` 에 모이고, *실행 회차* 는 `heartbeat_runs` 가 끊어 준다. `cost_events` 의 컬럼 구성이 의미심장하다 — `cached_input_tokens` 가 별도로 존재한다. 이는 Anthropic·OpenAI·Gemini 가 모두 채택한 **prompt caching** 가격 모델을 회계까지 끌어 올린 결과다(자세한 내용은 [docs/research/10-llm-cost-budgeting.md](../research/10-llm-cost-budgeting.md)).
...
표 2의 군집은 `server/src/services/`의 디렉터리 구조와도 거의 1:1로 대응된다. 즉, **데이터 도메인이 곧 서비스 도메인**이다. 새 기능을 추가할 때도 이 군집은 영향 범위를 판단하는 기준이 된다 — 예컨대 "정기 작업 기능을 보강한다"는 요구는 *Work* 도메인의 `routines` 보조 테이블 + `services/routines.ts` 묶음에 주로 한정되고, "비용 알림 채널을 추가한다"는 요구는 *Finance* + *Governance* 두 군집을 동시에 건드린다.
...
`DATABASE_URL` 이 비어 있으면 Paperclip 은 자동으로 `~/.paperclip/instances/default/db/` 에 **embedded PostgreSQL** 인스턴스를 띄운다. 이 패키지는 `package.json` 에서 `embedded-postgres@18.1.0-beta.16` 으로 핀(pin)되어 있고, 한 줄짜리 패치(`patches/embedded-postgres@*.patch`) 가 적용된다. PostgreSQL 17 (full native) 을 그대로 호스트하므로 PGlite (WASM) 보다 호환성이 좋다.
