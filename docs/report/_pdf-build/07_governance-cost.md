...
표 1의 6가지 권한은 *영향 반경*으로 정렬되어 있다 — 위에서 아래로 갈수록 단일 에이전트 → 회사 → 이슈/트리 → 단건 → 예산 → 결정 단위로 좁혀진다. 보드는 평소 *결정 override*만 봐도 되지만, 위에서 아래까지 모든 행이 *항상 가용*하다는 점이 거버넌스의 안전망이다. graceful cancel의 동작은 §4-pause-behavior가 정의한다 — *(1) signal current run, (2) grace period, (3) force-kill on timeout, (4) stop future heartbeats.*
