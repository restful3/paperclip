...
| **단일 assignee + atomic checkout** | `issues.checkout_run_id`를 중심으로 한 조건부 UPDATE (`server/src/services/issues.ts:3593-3736` — 의사코드는 02장 §5) | 동시성 문제를 데이터 모델 차원에서 해결. CRDT/낙관적 락 없이 다중 조건 UPDATE 한 번으로 lock + status + adopt. |
...
| 약점 | 어디서 드러나는가 | 영향 |
|---|---|---|
| **거대 단일 파일들** | `services/company-portability.ts` 180 KB, `routes/issues.ts` 168 KB, `ui/pages/IssueDetail.tsx` 160 KB, `routes/access.ts` 144 KB, `routes/agents.ts` 120 KB, `routes/plugins.ts` 92 KB | 가독성·테스트성 저해. 신규 기여자가 한 번에 구조를 파악하기 어렵다. 서비스 분할이 점진적 과제. |
